...
/* Byte API buffers need no alignment; lanes on the wire are little-endian. */
static uint32_t load_le32(const unsigned char *p)
{
    return (uint32_t)p[0] | (uint32_t)p[1] << 8 | (uint32_t)p[2] << 16 | (uint32_t)p[3] << 24;
}
static void store_le32(unsigned char *p, uint32_t value)
{
...
/* --- State type (replaces KeccakP-1600-SnP.h) --- */
typedef struct { uint32_t A[50]; } KeccakP1600_plain32_state;

/* --- SnP lane-split helpers (replaces SnP-Relaned.h) --- */
...
static void KeccakP1600_SetBytesInLaneToZero(KeccakP1600_plain32_state *state, unsigned int lanePosition, unsigned int offset, unsigned int length)
{
    ...
    low = load_le32(laneAsBytes);
    high = load_le32(laneAsBytes + 4);
...
void KeccakP1600_AddByte(void *state, unsigned char byte, unsigned int offset)
{
    ...
    uint32_t low, high;
    uint32_t temp, temp0, temp1;
    uint32_t *stateAsHalfLanes = ((KeccakP1600_plain32_state *)state)->A;
...
static void KeccakP1600_AddLanes(KeccakP1600_plain32_state *state, const unsigned char *data, unsigned int laneCount)
{
    for (unsigned i = 0; i < laneCount; ++i) {
        uint32_t low = load_le32(data + 8*i), high = load_le32(data + 8*i + 4);
        uint32_t t, x0, x1;
        toBitInterleavingAndXOR(low, high, state->A[2*i], state->A[2*i+1], t, x0, x1);
    }
}
...
static void KeccakP1600_OverwriteBytesInLane(KeccakP1600_plain32_state *state, unsigned int lanePosition, const unsigned char *data, unsigned int offset, unsigned int length)
{
...
static void KeccakP1600_OverwriteLanes(KeccakP1600_plain32_state *state, const unsigned char *data, unsigned int laneCount)
{
    for (unsigned i = 0; i < laneCount; ++i) {
        uint32_t low = load_le32(data + 8*i), high = load_le32(data + 8*i + 4);
        uint32_t t, x0, x1;
        toBitInterleavingAndSet(low, high, state->A[2*i], state->A[2*i+1], t, x0, x1);
    }
}
...
{
    SnP_OverwriteBytes((KeccakP1600_plain32_state *)state, data, offset, length, KeccakP1600_OverwriteLanes, KeccakP1600_OverwriteBytesInLane, 8);
}
...
void KeccakP1600_OverwriteWithZeroes(void *state, unsigned int byteCount)
{
    ...
    if (byteCount%8 != 0)
        KeccakP1600_SetBytesInLaneToZero((KeccakP1600_plain32_state *)state, byteCount/8, 0, byteCount%8);
}
...
static void KeccakP1600_ExtractBytesInLane(const KeccakP1600_plain32_state *state, unsigned int lanePosition, unsigned char *data, unsigned int offset, unsigned int length)
{
    ...
    store_le32(laneAsBytes, low);
    store_le32(laneAsBytes + 4, high);
...
{
    for (unsigned i = 0; i < laneCount; ++i) {
        uint32_t low, high, t, x0, x1;
        fromBitInterleaving(state->A[2*i], state->A[2*i+1], low, high, t, x0, x1);
        store_le32(data + 8*i, low);
        store_le32(data + 8*i + 4, high);
    }
}
...
{
    SnP_ExtractBytes((const KeccakP1600_plain32_state *)state, data, offset, length, KeccakP1600_ExtractLanes, KeccakP1600_ExtractBytesInLane, 8);
}

/* ---------------------------------------------------------------- */

static void KeccakP1600_ExtractAndAddBytesInLane(const KeccakP1600_plain32_state *state, unsigned int lanePosition, const unsigned char *input, unsigned char *output, unsigned int offset, unsigned int length)
{
    const uint32_t *stateAsHalfLanes = state->A;
    uint32_t low, high, temp, temp0, temp1;
    uint8_t laneAsBytes[8];
...
static void KeccakP1600_ExtractAndAddLanes(const KeccakP1600_plain32_state *state, const unsigned char *input, unsigned char *output, unsigned int laneCount)
{
    for (unsigned i = 0; i < laneCount; ++i) {
        uint32_t low, high, t, x0, x1;
        fromBitInterleaving(state->A[2*i], state->A[2*i+1], low, high, t, x0, x1);
        // Read both halves before writing, including exact in-place XOR.
        low ^= load_le32(input + 8*i);
        high ^= load_le32(input + 8*i + 4);
        store_le32(output + 8*i, low);
        store_le32(output + 8*i + 4, high);
    }
}
/* ---------------------------------------------------------------- */
...
{
    SnP_ExtractAndAddBytes((const KeccakP1600_plain32_state *)state, input, output, offset, length, KeccakP1600_ExtractAndAddLanes, KeccakP1600_ExtractAndAddBytesInLane, 8);
}
...
{
     KeccakP1600_Permute_Nrounds((KeccakP1600_plain32_state *)state, 12);
}
