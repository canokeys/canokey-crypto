...
static void test_rsa128_decrypt(void **state) {
  ...
  uint8_t q[] = {0x99, 0x84, 0x8e, 0xe6, 0x5d, 0x6c, 0xb8, 0xf9};
  uint8_t e[] = {0, 1, 0, 1};
  uint8_t dp[] = {0xb0, 0x58, 0x6f, 0x5a, 0x5d, 0x93, 0x12, 0x11};
  uint8_t dq[] = {0x7e, 0xd5, 0xb8, 0x42, 0xb7, 0xb9, 0x55, 0xf1};
  uint8_t qinv[] = {0xd0, 0xdc, 0x72, 0xe2, 0x41, 0x82, 0xd2, 0x60};
  ...
  assert_int_equal(rsa_private(&key, cipher_text, buf), 0);
  for (int i = 0; i != 16; ++i) {
    assert_int_equal(buf[i], expected[i]);
  }
}
